...
NAMESPACE="${NAMESPACE:-suma-ecommerce}"
DEPLOYMENT_NAME="${DEPLOYMENT_NAME:-suma-ecommerce-api}"
IMAGE_NAME="${IMAGE_NAME:-suma-ecommerce-api}"
...
service_root="$workspace_root/suma-ecommerce"
dockerfile="$service_root/dockerfile"
manifests_dir="$(dirname "${BASH_SOURCE[0]}")/suma-ecommerce"
...
for f in namespace.yaml pvc.yaml service.yaml deployment.yaml hpa.yaml; do
