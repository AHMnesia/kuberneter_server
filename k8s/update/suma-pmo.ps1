param(
    [string]$Namespace = 'suma-pmo',
    [string]$DeploymentName = 'suma-pmo-api',
    [string]$ImageName = 'suma-pmo-api',
...
Write-Host "=== Suma pmo: Build & Redeploy ===" -ForegroundColor Green
...
function Get-Paths {
    ...
    $serviceRoot = Join-Path $workspaceRoot 'suma-pmo'
    $dockerfile = Join-Path $serviceRoot 'dockerfile'
    $k8sSvcDir = Join-Path $k8sRoot 'suma-pmo'
...
Apply-pmoStack -Ns $Namespace -Dir $paths.Manifests
