param(
    [string]$Namespace = 'suma-android',
    [string]$DeploymentName = 'suma-android-api',
    [string]$ImageName = 'suma-android-api',
...
Write-Host "=== Suma android: Build & Redeploy ===" -ForegroundColor Green
...
function Get-Paths {
    ...
    $serviceRoot = Join-Path $workspaceRoot 'suma-android'
    $dockerfile = Join-Path $serviceRoot 'dockerfile'
    $k8sSvcDir = Join-Path $k8sRoot 'suma-android'
...
Apply-androidStack -Ns $Namespace -Dir $paths.Manifests
